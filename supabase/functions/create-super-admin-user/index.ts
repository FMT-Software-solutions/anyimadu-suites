...
serve(async (req) => {
  ...
  try {
    ...
    if (!emailRegex.test(email) || name.trim().length < 2) {
    ...
    const { error: updErr } = await adminClient.auth.admin.updateUserById(created.user.id, { app_metadata: { user_type: "admin", role: "super_admin", roles: ["super_admin"] } as any })
    if (updErr) {
    ...
    if (resendKey) {
      const { error: sendErr } = await resend.emails.send({
        from: fromEmail,
        to: [email],
        subject: "Your Anyimadu Suites super admin account",
        html: `
